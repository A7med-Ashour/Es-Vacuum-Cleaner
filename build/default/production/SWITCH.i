...
# 2 "<built-in>" 2
# 1 "SWITCH.c" 2
# 1 "./MICRO_CONTROLLER.h" 1
# 10 "./MICRO_CONTROLLER.h"
# 1 "./config.h" 1
...
# 17 "./config.h" 2
# 10 "./MICRO_CONTROLLER.h" 2
# 26 "./MICRO_CONTROLLER.h"
...
# 92 "./MICRO_CONTROLLER.h"
typedef unsigned char TMR0_SIZE;
# 156 "./MICRO_CONTROLLER.h"
...
static tSwitch_Info switches_data[(3)];
...
void switch_init(tSwitch sw){
    ...
    ((*(switches_data[sw].ctrl_reg)) = ((*(switches_data[sw].ctrl_reg) & (~(1 << switches_data[sw].pin_number))) | (((1)) << switches_data[sw].pin_number)));



    for (index = 0; index < (2);index++){

        switches_data[sw].samples[index] = ((1));
    }
    ...
    switches_data[sw].pressed_time = (0);
    switches_data[sw].pressed_counter = (0);
}
...
void switch_update(void){
    ...
    sw_update_counter = sw_update_counter + (10);



    if(sw_update_counter == (20)){

        sw_update_counter = (0);

        for(sw_index = sw_plus; sw_index < (3); sw_index++){


            for(sample_index = 0; sample_index < (2) - 1; sample_index++){
                switches_data[sw_index].samples[sample_index] = switches_data[sw_index].samples[sample_index + 1];
            }


            switches_data[sw_index].samples[(2) - 1] = ((*(switches_data[sw_index].data_reg) >> switches_data[sw_index].pin_number) & (1));




            if(switches_data[sw_index].state == sw_released &&
               switches_data[sw_index].samples[(2) - 2] == ((0)) &&
               switches_data[sw_index].samples[(2) - 1] == ((0))
               )
            {
                switches_data[sw_index].state = sw_prepressed;
            }

            else if(switches_data[sw_index].state == sw_prepressed && switches_data[sw_index].samples[(2) - 1] == ((0)))
            {
                switches_data[sw_index].state = sw_pressed;

            }

            else if(switches_data[sw_index].state == sw_pressed && switches_data[sw_index].samples[(2) - 1] == ((0))){

                switches_data[sw_index].pressed_counter += (20);

                if(switches_data[sw_index].pressed_counter == (1000)){
                    switches_data[sw_index].pressed_counter = (0);
                    if(switches_data[sw_index].pressed_time < (60)) {switches_data[sw_index].pressed_time++;}
                    else { }
                }else{ }
            }

            else if(switches_data[sw_index].state == sw_pressed &&
                    switches_data[sw_index].samples[(2) - 2] == ((1)) &&
                    switches_data[sw_index].samples[(2) - 1] == ((1))
                   )
                   {
                        switches_data[sw_index].state = sw_prereleased;

                        switches_data[sw_index].pressed_counter = (0);
                        switches_data[sw_index].pressed_time = (0);
                   }

            else if(switches_data[sw_index].state == sw_prereleased && switches_data[sw_index].samples[(2) - 1] == ((1)))
            {
...
static void switches_Schematic_init(void){
    ...
    switches_data[sw_plus].ctrl_reg = &(((*((volatile tRegister *)(0x86))).Reg));
    switches_data[sw_plus].data_reg = &(((*((volatile tRegister *)(0x06))).Reg));
    switches_data[sw_plus].pin_number = ((0));



    switches_data[sw_minus].ctrl_reg = &(((*((volatile tRegister *)(0x86))).Reg));
    switches_data[sw_minus].data_reg = &(((*((volatile tRegister *)(0x06))).Reg));
    switches_data[sw_minus].pin_number = ((1));



    switches_data[sw_dust].ctrl_reg = &(((*((volatile tRegister *)(0x86))).Reg));
    switches_data[sw_dust].data_reg = &(((*((volatile tRegister *)(0x06))).Reg));
    switches_data[sw_dust].pin_number = ((2));
