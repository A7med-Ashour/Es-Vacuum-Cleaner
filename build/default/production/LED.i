...
void LED_init(tLed led){
    ...
    (((*LedDB[led].dirReg) = ((*LedDB[led].dirReg & (~(1 << LedDB[led].pin))) | (((0)) << LedDB[led].pin))));
...
void LED_show(tLed led){
    ...
    (((*LedDB[led].dataReg) = ((*LedDB[led].dataReg & (~(1 << LedDB[led].pin))) | (LedDB[led].state << LedDB[led].pin))));
}

void LED_update(void) {

    static tByte LED_counter = (0);
    tLed index;
...
static void LED_create(void){
    ...
    LedDB[(0)] = (tLedObj) { &((*((volatile tRegister *)(0x86))).Reg) , &((*((volatile tRegister *)(0x06))).Reg) , (3) , LED_OFF };
}
